...
impl TextLayoutKey {
    pub fn new(
        ...
        style.font_axes.optical_size.map(f32::to_bits).hash(&mut hasher);
        ...
        style.font_axes.thick_stroke.map(f32::to_bits).hash(&mut hasher);
        style.font_axes.thin_stroke.map(f32::to_bits).hash(&mut hasher);
        style.font_axes.counter_width.map(f32::to_bits).hash(&mut hasher);
        style.font_axes.uppercase_height.map(f32::to_bits).hash(&mut hasher);
        style.font_axes.lowercase_height.map(f32::to_bits).hash(&mut hasher);
        style.font_axes.ascender_height.map(f32::to_bits).hash(&mut hasher);
        style.font_axes.descender_depth.map(f32::to_bits).hash(&mut hasher);
        style.font_axes.figure_height.map(f32::to_bits).hash(&mut hasher);
...
mod tests {
    ...
    fn entry_evicted_after_two_idle_frames() {
        ...
        assert_eq!(cache.len(), 0, "entry should be evicted after 2 idle frames");
    }
